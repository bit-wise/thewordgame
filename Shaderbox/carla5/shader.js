...
function setup() {
    ...
    // frag_shader_2.setUniform("wrap_x", true);
    // frag_shader_2.setUniform("wrap_y", true);
    // frag_shader_2.setUniform("mix", config.mix);
    // frag_shader_2.setUniform("offsets", [config.offsets.x, config.offsets.y, config.offsets.z]);
...
function draw() {
    ...
    if (config.mic_enabled) {
        ...
        // Reset scale
        config.scl_min = normalize(config.micMinS, config.micMinS, config.micMaxS);
        config.scl_lvl = normalize(config.micLvl, config.micMinS, config.micMaxS);
        config.scl_max = normalize(config.micMaxS, config.micMinS, config.micMaxS);
        ...
        // Prevent min zero
        if (config.micMin <= 0) {
            config.micMin = 0.000000001;
        }
        if (config.micMinS <= 0) {
            config.micMinS = 0.000000001;
        }
        ...
        config.fft_sumt = config.fft_sum;
        config.fft_sumx = Math.max(config.fft_sumx, config.fft_sumt)
        // config.fft_sum /= (spectrum.length ** Math.RT5);

        config.fft_maxx = Math.max(config.fft_maxx, config.fft_max);
        config.fft_sum /= config.fft_sumx;
        // config.fft_sum /= config.fft_maxx;
        // config.fft_sum *= 100;
        config.fft_sum /= Math.E;
        config.fft_sum /= Math.RT5;
        config.fft_sum /= Math.PHI;
        ...
        frag_shader_2.setUniform("noise", config.fft_sum);
